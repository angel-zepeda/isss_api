  ...
  Pensioner1.find({})
    ...
    .exec((err, pensioners1) => {
      if (err)
        return res
          .status(500)
          .send({ code: 500, meesage: 'Error en el servidor' });
      if (!pensioners1)
        return res.status(404).send({ code: 404, meesage: 'No hay registros' });
...
const create = (req, res) => {
  ...
  pensioner1.save((err, pensioner1Store) => {
    if (err)
      return res.send({ code: 500, message: 'Error en la petición: ' + err });
    if (!pensioner1Store)
      return res.send({ code: 404, message: 'No se pudo registrar los datos' });
...
const show = (req, res) => {
  const { id } = req.params;
  Pensioner1.findById(id).exec((err, pensioner) => {
    if (err)
      return res
        .status(500)
        .send({ code: 500, message: 'Error en la petición' });
    if (!pensioner)
      return res.status(400).send({ code: 404, message: 'No se pudo borrar' });
...
const deletePensioner1 = (req, res) => {
  ...
  Pensioner1.findByIdAndDelete(id).exec((err, deleted) => {
    if (err)
      return res
        .status(500)
        .send({ code: 500, message: 'Error en la peticion' });
    if (!deleted)
      return res.status(404).send({ code: 404, message: 'No se pudo borrar' });
...
const updatePensioner1 = (req, res) => {
  ...
  Pensioner1.findByIdAndUpdate(id, update, { new: true }, (err, update) => {
    ...
    if (!update)
      return res
        .status(404)
        .send({ code: 404, message: 'No se pudo actualizar' });
    return res
      .status(200)
      .send({ code: 200, message: 'Actulizado correctamente', update });
  });
};
